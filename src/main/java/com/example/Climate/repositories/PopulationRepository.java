...
public interface PopulationRepository extends JpaRepository<Population, Integer> {
    ...
    @Query("SELECT p FROM Population p WHERE p.country.countryName = 'World' ")
    ...
    @Query("SELECT p FROM Population p WHERE p.country.countryCode = :countryCode")
    ...
    @Query("""
            ...
            WHERE p1.country.countryCode = :countryCode AND p1.year = :endYear
            AND p2.country.countryCode = :countryCode  AND p2.year = :startYear
            """)
